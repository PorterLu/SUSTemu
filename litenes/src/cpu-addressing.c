...
void cpu_address_immediate() {
  op_value = memory_readb_fast(cpu.PC);
  cpu.PC++;
}

void cpu_address_zero_page() {
  op_address = memory_readb_fast(cpu.PC);
  op_value = CPU_RAM[op_address];
  cpu.PC++;
}

void cpu_address_zero_page_x() {
  op_address = (memory_readb_fast(cpu.PC) + cpu.X) & 0xFF;
  op_value = CPU_RAM[op_address];
  cpu.PC++;
}

void cpu_address_zero_page_y() {
  op_address = (memory_readb_fast(cpu.PC) + cpu.Y) & 0xFF;
  op_value = CPU_RAM[op_address];
  cpu.PC++;
}
...
void cpu_address_absolute() {
  ...
  op_value = memory_readb_fast(op_address);
  cpu.PC += 2;
}
...
void cpu_address_indirect() {
  ...
  if ((arg_addr & 0xFF) == 0xFF) {
    // Buggy code
    op_address = (memory_readb_fast(arg_addr & 0xFF00) << 8) + memory_readb_fast(arg_addr);
  }
  else {
    // Normal code
...
void cpu_address_indirect_x() {
  byte arg_addr = memory_readb_fast(cpu.PC);
  op_address = (memory_readb_fast((arg_addr + cpu.X + 1) & 0xFF) << 8) | memory_readb_fast((arg_addr + cpu.X) & 0xFF);
  op_value = memory_readb_fast(op_address);
  cpu.PC++;
}
...
void cpu_address_indirect_y() {
  byte arg_addr = memory_readb_fast(cpu.PC);
  op_address = (((memory_readb_fast((arg_addr + 1) & 0xFF) << 8) | memory_readb_fast(arg_addr)) + cpu.Y) & 0xFFFF;
  op_value = memory_readb_fast(op_address);
  cpu.PC++;
